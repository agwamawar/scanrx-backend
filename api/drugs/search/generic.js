/**
 * Generic Drug Search Endpoint
 * 
 * POST /api/drugs/search/generic
 * 
 * Searches EMDEX for generic name drugs.
 */

const { cachedEmdexRequest, EmdexError, CACHE_TTL } = require('../../services/emdex-service');
const { transformGenericResults } = require('../../services/drug-transformer');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Call EMDEX generic search with caching
    console.log(`[EMDEX] Searching generics for: "${searchQuery}"`);
    ...
    let results = transformGenericResults(emdexResponse);
    ...
    console.error('[EMDEX] Generic search error:', error);
